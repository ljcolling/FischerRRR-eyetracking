function ptb_fischer_rep_eyetracking()
...
% This is the version that is designed to work with the EyeLink 1000
% eye-tracker
...
EyeLinkDevMode = 0
if EyeLinkDevMode == 1
    EyeLinkStatus = Eyelink('InitializeDummy');
else
    EyeLinkStatus = Eyelink('Initialize');
end

if EyeLinkStatus ~= 0
    error('Something is wrong with the EyeLink')
end
%}
...
try
    ...
    if exist(paramsfile,'file') == 2
        ...
        error('Params.mat not found. Please run the calibration script (ptb_fischer_calibration.m)')
    end
    ...
    while t < length(trialStruct) + 1 %% comment for debugging
       ...
       % during the fixate time
        ...
        MustAcheiveStableFixationFor = .5; % seconds
        ...
        while Proceed == 0
            while  EndFixation == 0
                ...
                if infixationWindow(mx,my) %&& ~infix
                    ...
                    if abs(FixationCounterStart - NowTime) > MustAcheiveStableFixationFor
                        AcheivedFixation = 1;
                        EndFixation = 1;
                        FixatedFor = abs(FixationCounterStart - NowTime);
                    end
                    disp('in fix');
                    infix = 1;
            ...
            if AcheivedFixation == 0
                disp('Stable fixation could not be acheived')
                disp('We wil need to redo the calibration')
                %Screen('flip')
                Eyelink('command', 'button_function 5 "accept_target_fixation"');
                EyelinkDoTrackerSetup(el);
   ...
   % download data file
    ...
    save(subjectDataFile,'responseStruct','trialStruct','timings','params','systemParams',...
        'eyeTrackingStruct','-mat');
catch ME
    sca;
    ListenChar(0); %makes it so characters typed do show up in the command window
    warning(ME.message)
